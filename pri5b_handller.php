 ...
 */
 ...
 if(isset($_POST["submit"])){
            ...
            $select="SELECT email FROM pri5b_register WHERE email='$email'";
            if($query=mysqli_query($connect,$select)){
                $num=mysqli_num_rows($query);
                if($num > 0){
                    $msg="Email <b>$email</b> already exists!";
                    header("location:pri5b_form.php?msg=$msg");
                }else{
                        $insert="INSERT INTO pri5b_register(first_name,middle_name,surname,username,email,password,confirm_password,gender,age,class,address,registered_on,passport)VALUES('$firstname','$middlename','$surname','$username','$email','$password','$confirm_password','$gender','$age','$class','$address','$registered_on','$file')";
                        if(mysqli_query($connect,$insert)){
                            $msg="Data submitted successfully. Please Print Out this form because it will be require from you during clearance";
                             header("location:pri5b_fetch_form.php?msg=$msg");
                        }else{
