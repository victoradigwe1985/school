...
if (isset($_POST["delete"])&&!empty($_POST["delete"])){
    ...
    $id=$_POST["delete"];
    $select="SELECT register_id FROM js2c_register WHERE register_id='$id'";
    if($query=mysqli_query($connect,$select)){
        $getnum=mysqli_num_rows($query);
        if($getnum>0){
            $del="DELETE FROM js2c_register WHERE register_id='$id'";
            if($query=mysqli_query($connect,$del)){
                ?>
<script type="text/javascript">
    alert("Data Deleted Successfully");
    window.location="jss2c_fetchingdata.php";
    </script>
    <?php
            }else{
                ?>
