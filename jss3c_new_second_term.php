 ...
 */
 ...
 if(isset($_POST["submit"])){
         ...
         $select="SELECT stud_id FROM jss3c_second_schoolfee WHERE stud_id='$stud_id'";
            if($query=mysqli_query($connect,$select)){
                $num=mysqli_num_rows($query);
                if($num > 0){
                    $msg="Student ID <b>$stud_id </b> already exists!";
                    header("location:jss3c_new_second_term.php?msg=$msg");
                }else{
                        $insert="INSERT INTO jss3c_second_schoolfee(stud_id,fullname,firstpay,secondpay,fullpay,total)VALUES('$stud_id','$fullname','$firstpay','$secondpay','$fullpay','$total')";
                        if(mysqli_query($connect,$insert)){
                            $msg="School Fee submitted successfully !";
                             header("location:jss3c_new_second_term.php?msg=$msg");
                        }else{
                            $msg="failed to exercute query on the database!";
                             header("location:jss3c_new_first_term.php?msg=$msg");
                        }
                }
            }
 }
   ...
   color:white;
    ...
    <body>
        ...
        <a href="class_selector_new.php" class="btn btn-lg btn-success" style="border-color: gray;  font-size: 18px;  margin-top: 10px; color:white;"> <<< Go Back</a><br>
         <a href="jss3c_second_existing.php" class="btn btn-lg btn-success" style="border-color: gray;  font-size: 18px;  margin-top: 10px; color:white;">View Records</a><br>
        </div>
      ...
      <input type="text" name="fullpay" class="form-control input-lg"  placeholder="" style="width:110px;">
       <button type="submit"  name="submit" class="btn btn-gray" id="submit">Submit</button>
           ...
           <div style="background-color:lightgray; width: 350px; padding: 5px;"> <a href="jss3c_new_second_term.php"><button value="OK" class="btn btn-gray" id="submit">OK</button></a> <a href="jss3c_second_existing.php"><button value="View Records" class="btn btn-gray" id="submit">View Records</button></a></div>  
</div>
