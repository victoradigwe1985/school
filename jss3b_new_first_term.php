 ...
 */
 ...
 if(isset($_POST["submit"])){
         ...
         $select="SELECT stud_id FROM jss3b_schoolfee WHERE stud_id='$stud_id'";
            if($query=mysqli_query($connect,$select)){
                $num=mysqli_num_rows($query);
                if($num > 0){
                    $msg="Student ID <b>$stud_id </b> already exists!";
                    header("location:jss3b_new.php?msg=$msg");
                }else{
                        $insert="INSERT INTO jss3b_schoolfee(stud_id,fullname,firstpay,secondpay,fullpay,total)VALUES('$stud_id','$fullname','$firstpay','$secondpay','$fullpay','$total')";
...
#submit:hover{
   ...
   color:white;
    ...
    <body>
        ...
        <a href="jss3b_existing.php" class="btn btn-lg btn-success" style="border-color: gray;  font-size: 18px;  margin-top: 10px; color:white;">View Records</a><br>
        </div>
  ...
  <div style="background-color:lightgray; width: 350px; padding: 5px;"> <a href="jss3b_new_first_term.php"><button value="OK" class="btn btn-gray" id="submit">OK</button></a> <a href="jss3b_existing.php"><button value="View Records" class="btn btn-gray" id="submit">View Records</button></a></div>
</div>
