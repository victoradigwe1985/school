...
if (isset($_POST["delete"])&&!empty($_POST["delete"])){
    ...
    $id=$_POST["delete"];
    $select="SELECT register_id FROM pri6a_schoolfee WHERE register_id='$id'";
    if($query=mysqli_query($connect,$select)){
        $getnum=mysqli_num_rows($query);
        if($getnum>0){
            $del="DELETE FROM pri6a_schoolfee WHERE register_id='$id'";
            if($query=mysqli_query($connect,$del)){
                ?>
<script type="text/javascript">
    alert("Data Deleted Successfully");
    window.location="pri6a_existing.php";
    </script>
    <?php
            }else{
                ?>
