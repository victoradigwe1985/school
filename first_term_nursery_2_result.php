 <?php
           include_once("connection.php");
            ...
            $select="SELECT *FROM nursery_2_first_term_scores WHERE admission_no='$id'";
            $query=mysqli_query($connect,$select);
                while($row=mysqli_fetch_array($query)){
                 ...
                 $Average=$row["Average"];
                  ...
                  $select="SELECT *FROM nursery_2_assessment_first_register WHERE admission_no='$id'";
            $query=mysqli_query($connect,$select);
                while($row=mysqli_fetch_array($query)){
...
<table cellpadding="15px" width="100%" border="1px" cellspacing="0px" style="font-size: 15px; line-height:1.9em;">
        <thead>
            ...
            <th style="width:15%">Age:<?php echo  '3+';?></th>
