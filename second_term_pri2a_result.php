 <?php
           include_once("connection.php");
            ...
            $select="SELECT *FROM primary2a_second_term_scores WHERE admission_no='$id'";
            $query=mysqli_query($connect,$select);
                while($row=mysqli_fetch_array($query)){
                 ...
                 $Average=$row["Average"];
                  ...
                  $select="SELECT *FROM pri2a_assessment_second_register WHERE admission_no='$id'";
            $query=mysqli_query($connect,$select);
                while($row=mysqli_fetch_array($query)){
...
<table cellpadding="15px" width="100%" border="1px" cellspacing="0px" style="font-size: 15px; line-height:1.5em;">
        <thead>
            ...
            <th style="width:15%">Age: <?php echo  '7+';?></th>
            <th style="width:25%">Class: Primary 2A</th>
            </tr>
    </thead>
            </table>
